...
def set_output(key, value):
	try:
		...
		print(f"Error setting output {key}: {str(e)}", file=sys.stderr)
		sys.exit(1)
...
try:
	...
	if not event_name:
		print("Error: GITHUB_EVENT_NAME environment variable is not set", file=sys.stderr)
		sys.exit(1)

	publish_hostport = os.getenv('PUBLISH_HOSTPORT')
	if not publish_hostport:
		print("Warning: PUBLISH_HOSTPORT environment variable is not set", file=sys.stderr)
	...
	try:
		...
		print(f"Error running meson setup: {str(e)}", file=sys.stderr)
		sys.exit(1)
	...
	try:
		...
		print(f"Error reading build options: {str(e)}", file=sys.stderr)
		sys.exit(1)
	...
	try:
		...
		print(f"Error processing mod_id: {str(e)}", file=sys.stderr)
		sys.exit(1)
	...
	if mod_id == 0:
		try:
			...
			print(f"Version mismatch: {str(e)}", file=sys.stderr)
			sys.exit(1)
	...
	print(f"Unexpected error in prepare.py: {str(e)}", file=sys.stderr)
	sys.exit(1)
